  ...
  <div class="navbar navbar-dark bg-dark shadow-sm">
    ...
    <% } else { %>
        ...
        <% } else if (permission != null && permission.equals("3")) { %>
            ...
            <li class="nav-item">
                <a class="nav-link" href="/SHOP/user/Question.jsp">문의내역</a>
            </li>
        <% } %>
            ...
            <tbody>
               <%
                ...
                <tr>
                    ...
                    <td> <%= product_name %></td>
					<%if (chatstatus.equals("새로운 메시지가 도착하였습니다.")){ %>
                    <td onclick="location.href='addQuestion.jsp?customerName=<%=customer_name%>&customerId=<%=customerId%>&productId=<%=productId%>&productImage=<%=product_image1%>&productName=<%=product_name%>'"> 채팅문의보냄 </td>
					<% }else if(chatstatus.equals("답변을 하였습니다.")){%>
					<td onclick="location.href='addQuestion.jsp?customerName=<%=customer_name%>&customerId=<%=customerId%>&productId=<%=productId%>&productImage=<%=product_image1%>&productName=<%=product_name%>'"> 채팅답변옴 </td>
      				<%}else{ %>
      				<td onclick="location.href='addQuestion.jsp?customerName=<%=customer_name%>&customerId=<%=customerId%>&productId=<%=productId%>&productImage=<%=product_image1%>&productName=<%=product_name%>'"> 문의 종료</td>
      				<%} %>
      				<div>
