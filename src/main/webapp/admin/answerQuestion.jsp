	...
	%>
     <div class="container">
        ...
        <form id="chatForm">
            ...
            <button type="button" id="sendMessage" class="btn btn-primary">답변하기</button>
            <button type="button" id="endanswer" class="btn btn-warning">답변종료</button>
        </form>
    </div>
...
document.getElementById('endanswer').addEventListener('click', function() {
    var customerId = '<%= customerId %>';
    var productId = '<%= productId %>';
    console.log("End answer button clicked"); // 디버깅을 위해 콘솔 로그 추가
    $.ajax({
        type: 'POST',
        url: 'endAnswer_process.jsp',
        data: {
            customerId: customerId,
            productId: productId
        },
        success: function(response) {
            console.log("Response:", response); // 디버깅을 위해 콘솔 로그 추가
            window.location.href = 'allQuestion.jsp';
        },
        error: function(error) {
            console.log(error);
            alert('오류가 발생했습니다.');
        ...
        success: function(response) {
            ...
            messages.forEach(function(message) {
                ...
                if (message.customerId === '1') { // Admin messages
                	...
                	messageElement.textContent = <%= customerId%>+"번 고객:"+message.message;
                    messageElement.style.background = 'lightblue';
                    messageElement.style.marginRight = 'auto';
                    messageElement.style.textAlign = 'left';
                }
