...
<%
    ...
    ResultSet countRs = countStmt.executeQuery("SELECT COUNT(*) AS total FROM orders where status = '주문 완료'");
    countRs.next();
               ...
               <%
                    try {
                        ...
                        PreparedStatement stmt = con.prepareStatement(
                        		"SELECT * FROM orders,customer,product where customer.customer_id = orders.customer_id AND product.product_id = orders.product_id AND status = '주문 완료' LIMIT ?, ?");
                        stmt.setInt(1, start);
