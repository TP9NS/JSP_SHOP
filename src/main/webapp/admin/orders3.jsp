...
<%
    ...
    ResultSet countRs = countStmt.executeQuery("SELECT COUNT(*) AS total FROM orders WHERE status = '배송 중'");
    countRs.next();
               ...
               <%
                    try {
                        ...
                        PreparedStatement stmt = con.prepareStatement(
                        		"SELECT * FROM orders,customer,product where customer.customer_id = orders.customer_id AND product.product_id = orders.product_id AND status = '배송 중' LIMIT ?, ?");
                        stmt.setInt(1, start);
